...
const ScrollSection = ({ title, text, imageUrl, isReverse, index, highlightWords = [] }) => {
  ...
  return (
    <section
      ref={containerRef}
      className={`sticky top-0 h-screen w-full scroll-m-20 bg-black flex items-center justify-center overflow-hidden border-[3px] ${
        index > 0 ? "rounded-t-[50px] lg:rounded-t-[100px]" : ""
      }`}
      style={{ 
        zIndex: index + 1,
        borderColor: '#86D276'
    ...
    >
      <div
        className={`flex flex-col lg:flex-row w-full font-base max-w-[1600px] h-full items-center gap-6 lg:gap-12 px-6 py-10 lg:px-12 lg:py-0 ${
